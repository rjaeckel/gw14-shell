 ...
 * @method static apiResult|mixed setCertificate ( mixed $data=null ,string $queryString=null ) <p>request: PUT system/directories/{directoryId}/certificate</p><p>template-var: directoryId => xs:string : template</p>
 * @method static apiResult|xsd\apiStatus testConnection ( xsd\directory $data=null ,string $queryString=null ) <p>request: POST system/directories/{directoryId}/test</p><p>template-var: directoryId => xs:string : template</p><p>query-String: attrs => xs:string : query</p>
...
class directory extends wadlProxy { 
	 ...
	 * @return apiResult|mixed
	 *
	 * <p>request: PUT system/directories/{directoryId}/certificate</p>
	 * <p>template-var: directoryId => xs:string : template</p>
	 */
	...
	/**
	 * <p>Test the LDAP connection for an existing or new directory using an  optional list of modifications. To test an existing directory, supply  the name of the directory in the URL path and an empty object (i.e. "{}"  for JSON) in the request body.</p>  <p>To test an existing directory with modifications  (i.e. test a new password for an existing directory before saving changes),  supply an object in the request entity that contains the modifications (i.e.  {"syncPassword": "newPassword"}). Note that the changes will be used for the  test but will NOT be committed to the directory object.</p>  <p>To test a new directory (one that has not yet been created), supply the  directory name ".new" in the directory path (i.e. /system/directories/.new/test)  and put all of the required parameters in a directory object in the request entity.  </p>
	 * @internal
	 *
	 * @param apiResult|xsd\directory $data user data; may repeat which will result in a merge
	 * @param string $queryString a whole query-string or one part like var=value; may repeat
	 * @return apiResult|xsd\apiStatus
	 *
	 * <p>request: POST system/directories/{directoryId}/test</p>
	 * <p>template-var: directoryId => xs:string : template</p>
	 * <p>query-String: attrs => xs:string : query</p>
	 */
	protected function _testConnection ($data=null,$queryString=null) { 
		return $this->doRequest ('testConnection',$data,$queryString);
	}
	/**
