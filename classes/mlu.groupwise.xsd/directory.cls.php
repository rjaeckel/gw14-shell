<?php
namespace mlu\groupwise\xsd;
/**
  * XSD-abstracted interfaces...
  */
abstract class directory extends restAbstractLdapServer{
	/**
	 * @var string $adminDefined1
	 * minOccurs: 0
	 */
	public $adminDefined1;
	/**
	 * @var string $adminDefined10
	 * minOccurs: 0
	 */
	public $adminDefined10;
	/**
	 * @var string $adminDefined11
	 * minOccurs: 0
	 */
	public $adminDefined11;
	/**
	 * @var string $adminDefined12
	 * minOccurs: 0
	 */
	public $adminDefined12;
	/**
	 * @var string $adminDefined13
	 * minOccurs: 0
	 */
	public $adminDefined13;
	/**
	 * @var string $adminDefined14
	 * minOccurs: 0
	 */
	public $adminDefined14;
	/**
	 * @var string $adminDefined15
	 * minOccurs: 0
	 */
	public $adminDefined15;
	/**
	 * @var string $adminDefined16
	 * minOccurs: 0
	 */
	public $adminDefined16;
	/**
	 * @var string $adminDefined17
	 * minOccurs: 0
	 */
	public $adminDefined17;
	/**
	 * @var string $adminDefined18
	 * minOccurs: 0
	 */
	public $adminDefined18;
	/**
	 * @var string $adminDefined19
	 * minOccurs: 0
	 */
	public $adminDefined19;
	/**
	 * @var string $adminDefined2
	 * minOccurs: 0
	 */
	public $adminDefined2;
	/**
	 * @var string $adminDefined20
	 * minOccurs: 0
	 */
	public $adminDefined20;
	/**
	 * @var string $adminDefined3
	 * minOccurs: 0
	 */
	public $adminDefined3;
	/**
	 * @var string $adminDefined4
	 * minOccurs: 0
	 */
	public $adminDefined4;
	/**
	 * @var string $adminDefined5
	 * minOccurs: 0
	 */
	public $adminDefined5;
	/**
	 * @var string $adminDefined6
	 * minOccurs: 0
	 */
	public $adminDefined6;
	/**
	 * @var string $adminDefined7
	 * minOccurs: 0
	 */
	public $adminDefined7;
	/**
	 * @var string $adminDefined8
	 * minOccurs: 0
	 */
	public $adminDefined8;
	/**
	 * @var string $adminDefined9
	 * minOccurs: 0
	 */
	public $adminDefined9;
	/**
	 * @var boolean $authDisablePasswordChange
	 * minOccurs: 0
	 */
	public $authDisablePasswordChange;
	/**
	 * @var string $authPassword
	 * minOccurs: 0
	 */
	public $authPassword;
	/**
	 * @var string $authUser
	 * minOccurs: 0
	 */
	public $authUser;
	/**
	 * @var string $baseDn
	 * minOccurs: 0
	 */
	public $baseDn;
	/**
	 * @var directoryType $directoryType
	 * minOccurs: 0
	 */
	public $directoryType;
	/**
	 * @var restEffectiveEmailPublish $emailPublish
	 * minOccurs: 0
	 */
	public $emailPublish;
	/**
	 * @var boolean $emailPublishEnabled
	 * minOccurs: 0
	 */
	public $emailPublishEnabled;
	/**
	 * @var restEffectiveEmailPublishFormats $emailPublishFormats
	 * minOccurs: 0
	 */
	public $emailPublishFormats;
	/**
	 * @var boolean $hasAuthPassword
	 * minOccurs: 0
	 */
	public $hasAuthPassword;
	/**
	 * @var boolean $hasSyncPassword
	 * minOccurs: 0
	 */
	public $hasSyncPassword;
	/**
	 * @var string $syncDomain
	 * minOccurs: 0
	 */
	public $syncDomain;
	/**
	 * @var boolean $syncEnabled
	 * minOccurs: 0
	 */
	public $syncEnabled;
	/**
	 * @var string $syncPassword
	 * minOccurs: 0
	 */
	public $syncPassword;
	/**
	 * @var string $syncUser
	 * minOccurs: 0
	 */
	public $syncUser;
	/**
	 * @var string $treeName
	 * minOccurs: 0
	 */
	public $treeName;
	/**
	 * @var ldapAuthMethod $userAuthenticationMethod
	 * minOccurs: 0
	 */
	public $userAuthenticationMethod;
}
