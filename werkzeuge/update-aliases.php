...
$list = array(
    ...
    'groups'=>'internetdomainname=rektorat.uni-halle.de', #aliases
    'users'=>'internetdomainname=rektorat.uni-halle.de',
...
foreach($list as $fn=>$filter) {
    try {
        ...
        do {
            ...
            //$removes=array();
            if ($l('object', false)) {
                $l->each(function ($item) use (&$worker) {
                    $worker->enqueue(function()use($item){
			//print_r(@mlu\common::putEffectiveValues((object)null,array('internetdomainname'=>'groupwise.uni-halle.de')));
			echo $item('id')."\n";
			$item->url(@PUT,@mlu\common::putEffectiveValues((object)null,array('internetdomainname'=>null)));
		    },worker::AUTOSTART);
                });
